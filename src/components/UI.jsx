...
export const inp = {
  ...
  boxSizing:"border-box", background:C.white, color:C.gray900,
  transition:"border-color 0.15s",
};

export const Field = ({label,children,hint}) => (
  <div>
    <div style={{fontSize:11,fontWeight:500,color:C.gray500,marginBottom:4,textTransform:"uppercase",letterSpacing:0.5}}>{label}</div>
    {children}
...
export const Sec = ({title,children}) => (
  <div style={{marginBottom:20}}>
    <div style={{fontSize:11,fontWeight:600,color:C.red,marginBottom:10,textTransform:"uppercase",letterSpacing:1}}>{title}</div>
    {children}
  </div>
);
...
export const Stat = ({label,value,sub,color,alert,trend}) => (
  <div style={{background:C.white,border:`1.5px solid ${alert?C.red:C.gray200}`,borderRadius:12,padding:"16px 18px",position:"relative",overflow:"hidden"}}>
    {alert&&<div style={{position:"absolute",top:0,left:0,right:0,height:3,background:C.red,borderRadius:"12px 12px 0 0"}}/>}
    <div style={{fontSize:11,fontWeight:500,color:C.gray400,marginBottom:6,textTransform:"uppercase",letterSpacing:0.5}}>{label}</div>
    <div style={{fontSize:22,fontWeight:700,color:color||C.gray900,lineHeight:1}}>{value}</div>
    {sub&&<div style={{fontSize:11,color:C.gray400,marginTop:6}}>{sub}</div>}
    {trend!==undefined&&(
...
export const Card = ({children,style={},onClick}) => (
  <div onClick={onClick} style={{background:C.white,border:`1.5px solid ${C.gray200}`,borderRadius:12,padding:"16px 18px",cursor:onClick?"pointer":"default",transition:"border-color 0.15s, box-shadow 0.15s",...style}}
    onMouseEnter={onClick?e=>{e.currentTarget.style.borderColor=C.gray300;e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.06)";}:null}
    onMouseLeave={onClick?e=>{e.currentTarget.style.borderColor=C.gray200;e.currentTarget.style.boxShadow="none";}:null}>
    {children}
  </div>
);
...
export const Btn = ({children,onClick,variant="outline",size="md",disabled,style={}}) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{...sizes[size],...variants[variant],borderRadius:8,fontWeight:500,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.5:1,display:"inline-flex",alignItems:"center",gap:6,transition:"all 0.15s",...style}}
      onMouseEnter={!disabled?e=>{if(variant==="primary")e.currentTarget.style.background=C.redDark;if(variant==="outline")e.currentTarget.style.background=C.gray50;}:null}
      onMouseLeave={!disabled?e=>{if(variant==="primary")e.currentTarget.style.background=C.red;if(variant==="outline")e.currentTarget.style.background="transparent";}:null}>
      {children}
    </button>
  );
};
...
export const Modal = ({children,onClose,maxWidth=640}) => (
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem",backdropFilter:"blur(2px)"}} onClick={onClose}>
    <div style={{background:C.white,borderRadius:16,width:"100%",maxWidth,maxHeight:"88vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}} onClick={e=>e.stopPropagation()}>
      {children}
    </div>
  </div>
);
...
export const ModalHeader = ({title,subtitle,onClose}) => (
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"20px 24px 16px",borderBottom:`1px solid ${C.gray100}`}}>
    <div>
      <div style={{fontSize:17,fontWeight:700,color:C.gray900}}>{title}</div>
      {subtitle&&<div style={{fontSize:12,color:C.gray400,marginTop:2}}>{subtitle}</div>}
    </div>
    <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",color:C.gray400,fontSize:20,lineHeight:1,padding:"2px 4px",borderRadius:6}}>×</button>
  </div>
);
...
export const ModalFooter = ({children}) => (
  <div style={{display:"flex",gap:10,justifyContent:"flex-end",padding:"16px 24px",borderTop:`1px solid ${C.gray100}`}}>
    {children}
  </div>
);

export const Table = ({cols,rows,emptyMsg="Sin datos"}) => (
  <div style={{background:C.white,border:`1.5px solid ${C.gray200}`,borderRadius:12,overflow:"hidden"}}>
    <div style={{overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:cols.length*100}}>
        <thead>
          <tr style={{background:C.gray900}}>
            {cols.map(c=><th key={c.key||c} style={{padding:"11px 14px",textAlign:"left",fontWeight:500,fontSize:11,color:C.gray300,textTransform:"uppercase",letterSpacing:0.5,whiteSpace:"nowrap"}}>{c.label||c}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.length===0&&<tr><td colSpan={cols.length} style={{padding:"2rem",textAlign:"center",color:C.gray400,fontSize:13}}>{emptyMsg}</td></tr>}
          {rows.map((row,i)=>(
...
export const EmptyState = ({icon="📭",title,desc,action}) => (
  <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"4rem 2rem",textAlign:"center"}}>
    ...
    <div style={{fontSize:15,fontWeight:600,color:C.gray700,marginBottom:6}}>{title}</div>
    {desc&&<div style={{fontSize:13,color:C.gray400,marginBottom:16}}>{desc}</div>}
    {action}
  </div>
);
